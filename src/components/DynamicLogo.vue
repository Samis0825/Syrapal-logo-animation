...
const logos = reactive([
	{ label: "kazimierz", url: "/src/assets/logo_aa.png" },
]);
		...
		img.onload = () => {
			...
			for (let y = 0; y < imgH; y += 4) {
				for (let x = 0; x < imgW; x += 4) {
					...
					console.log(
						"%c [ this.particleData ]-155",
						"font-size:13px; background:pink; color:#bf2c9f;",
						this.particleData.length
					);
					setTimeout(() => {
						if (particleCanvas.value && !activeLogo.value) {
							particleCanvas.value.changeImg(logoImgs[0]);
							activeLogo.value = logoImgs[0];
						}
					}, 0);
				}
			}
